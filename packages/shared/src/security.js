...
const escapeHtml = (value) => value.replace(/[&<>"'`]/g, (char) => HTML_ESCAPE_ENTITIES[char] || char);
...
export const sanitizeHTML = (html) => {
    ...
    let lastIndex = 0;
    let result = '';
    source.replace(TAG_REGEX, (match, tagName, attrsPart, offset) => {
        ...
        attrsPart.replace(ATTR_REGEX, (_attrMatch, rawAttr, _valueWithQuotes, doubleQuoted, singleQuoted) => {
...
export class RateLimiter {
    ...
    checkLimit(identifier, maxAttempts, windowMs) {
        const now = Date.now();
